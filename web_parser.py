...
api_url = os.getenv('REN3_API_URL', 'https://backend.ren3.ai')
user_uuid = os.getenv('REN3_USER_ID')
workspace_id = os.getenv('REN3_WORKSPACE_ID')
poll_interval = int(os.getenv('POLL_INTERVAL', '15'))
max_retries = int(os.getenv('MAX_RETRIES', '3'))
agent_uuid = os.getenv('WEB_AGENT_UUID')
agent_folder = os.getenv('WEB_AGENT_FOLDER')
...
def call_api(json_file_path, user_uuid, workspace_id):
    ...
    for attempt in range(max_retries + 1):  # Loop 3 times (0, 1, 2)
        try:
            ...
            print(f"\n=== DEBUG: Output Files ===")
            print(f"Found {len(output_files)} file(s) in output folder:")
            for idx, file in enumerate(output_files, 1):
                print(f"  File {idx}:")
                print(f"    Filename: {file.get('doc_filename')}")
                print(f"    UUID: {file.get('uuid')}")
