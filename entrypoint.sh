...
0 0 * * * cd /app && /usr/bin/python3 casino_scraper.py >> /app/logs/scraper_$(date +\%Y\%m\%d_\%H\%M\%S).log 2>&1 && sleep 5 && /usr/bin/python3 /app/web_parser.py >> /app/logs/processor_$(date +\%Y\%m\%d_\%H\%M\%S).log 2>&1 && sleep 5 && /usr/bin/python3 /app/jackpot_parser.py >> /app/logs/jackpot_$(date +\%Y\%m\%d_\%H\%M\%S).log 2>&1 && sleep 5 && /usr/bin/python3 calendar_generator.py >> /app/logs/calendar_$(date +\%Y\%m\%d_\%H\%M\%S).log 2>&1

0 8 * * * cd /app && /usr/bin/python3 email_script.py >> /app/logs/email_$(date +\%Y\%m\%d_\%H\%M\%S).log 2>&1 
